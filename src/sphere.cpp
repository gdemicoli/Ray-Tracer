...
#include "HitRecord.h"
#include "Materials.h"
Sphere::Sphere(Vec3 centre, double radius, Materials materials)
    : centre(centre), radius(radius), materials(materials)
{
}
...
HitRecord Sphere::hit(const Ray &ray)
{
    ...
    {
        return HitRecord{false, -1, Vec3(0, 0, 0), Vec3(0, 0, 0), materials};
    }
    ...
    {
        return HitRecord{true, t2, (ray.at(t2) - centre).normalise(), ray.at(t2), materials};
    }

    else if (t1 > 0) // inside sphere
    {
        return HitRecord{true, t1, (ray.at(t1) - centre).normalise(), ray.at(t1), materials};
    }

    else // sphere is behind, both are negative
    {
        return HitRecord{false, -1, (ray.at(t1) - centre).normalise(), ray.at(t1), materials};
    }
}